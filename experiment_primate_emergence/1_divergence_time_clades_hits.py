#########
# Input: (1) clades_divergence_time.csv table
...
# protein_type_list choices: "random", "cgc"
sequence_type_list = ["cds"]
protein_type_list = ["cgc724","random100","random60","random180"]
# add custom suffix to result filename
suffix = ''

df_clades_hits_fn = 'All_clades_gene_hits_4nonprimates0s_'  
if high_thre_only:
...
df_clades = pd.read_csv("clades_divergence_time_strict.csv")
...
for sequence_type in sequence_type_list:
    for protein_type in protein_type_list:
        ...
        fn = f"tblastn_summary_id80_cover80_cds_{protein_type}_proper_set_4nonprimates0s.csv"
...
df_clades_hits['count_cds_randomAll'] = df_clades_hits['count_cds_random60'] + df_clades_hits['count_cds_random180'] + df_clades_hits['count_cds_random100']
